defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    tonic_index = Enum.find_index(scale, fn(x) -> x == tonic end)
    Enum.at(scale, rem(tonic_index + @step[step], length(scale)))
  end
  ...
  def chromatic_scale(tonic \\ "C") do
    ...
    tonic_index = Enum.find_index(chromatic, fn(x) -> x == tonic end)
    Enum.slice(chromatic, tonic_index..-1) ++ Enum.slice(chromatic, 0..tonic_index)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    ...
    tonic_index = Enum.find_index(flat_chromatic, fn(x) -> x == tonic end)
    Enum.slice(flat_chromatic, tonic_index..-1) ++ Enum.slice(flat_chromatic, 0..tonic_index)
  end
  ...
  def scale(tonic, pattern) do
    ...
    indexes =
      pattern
      |> String.graphemes
      |> Enum.scan(0, fn(i, acc) -> acc + @step[i] end)
    for index <- [0 | indexes], do: Enum.at(chromatic, index)
  end
end
